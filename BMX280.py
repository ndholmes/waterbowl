...
class BMX280:
  bus_number  = 1
  i2c_address = 0x76
  elevation_meters = 1953 # My house
  digT = []
  digP = []
  digH = []
  ...
  def convert(self):
    ...
    self.t = self.compensate_T(temp_raw)
    self.p = self.compensate_P(pres_raw)
    self.h = self.compensate_H(hum_raw)
    
    return {'temperature_c': self.getTemperatureC(), 'temperature_f': self.getTemperatureF(), 'pressure': self.getPressure(), 'humidity':self.getHumidity(), 'pressure_sl':self.getSeaLevelPressure() }
 
  def getTemperatureC(self):
    return self.t

  def getTemperatureK(self):
    return self.getTemperatureC() + 273.15

  def getTemperatureF(self):
    return self.t * 1.8 + 32.0
  ...
  def getSeaLevelPressure(self, altitude = None):
    if None == altitude:
      altitude = self.elevation_meters
    elevation = 0.0065 * altitude
    compensationFactor = math.pow( 1 - ( elevation / (self.getTemperatureK() + elevation)), -5.257 )
    psl = self.getPressure() * 100.0 * compensationFactor
  ...
  def setup(self):
    ...
    config_reg    = (t_sb << 5) | (bmeFilter << 2) | spi3w_en
    ctrl_hum_reg  = osrs_h
  ...
  def __init__(self, busNum = 1, address=0x76):
    self.bus_number = busNum
